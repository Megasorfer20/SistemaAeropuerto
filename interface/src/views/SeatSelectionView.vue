...
export default {
  ...
  methods: {
    cargarVueloBackend() {
      ...
      // LLAMADA DOBLE: Traer info del vuelo Y sillas ocupadas
      Promise.all([
          window.pywebview.api.buscarVuelos({ id: vueloId }),
          window.pywebview.api.obtener_sillas_ocupadas(vueloId)
      ])
      .then(([resVuelos, resOcupadas]) => {
          if (resVuelos && resVuelos.length > 0) {
            this.vuelo = resVuelos[0]
            // Pasamos las sillas ocupadas reales a la función generadora
            this.generarAsientos(resOcupadas || []) 
          } else {
    ...
    // RECIBE LA LISTA DE OCUPADAS
    generarAsientos(sillasOcupadasIds) {
      ...
      for (let fila = 1; fila <= filasVIP; fila++) {
        for (let i = 0; i < asientosPorFila; i++) {
          ...
          const idAsiento = `VIP-${fila}${letras[i]}`
          
          this.asientosVIP.push({
            id: idAsiento,
            numero: `${fila}${letras[i]}`,
            tipo: 'vip',
            precio: Number(this.PRECIO_VIP), // ASEGURAR QUE ES NÚMERO (Solución Error #3)
            // VERIFICAR SI ESTÁ EN LA LISTA REAL (Solución Error #1)
            ocupado: sillasOcupadasIds.includes(idAsiento), 
          })
        }
      }
      ...
      for (let fila = filaInicioEco; fila < filaInicioEco + filasEco; fila++) {
        for (let i = 0; i < asientosPorFila; i++) {
          ...
          const idAsiento = `ECO-${fila}${letras[i]}`

          this.asientosNormales.push({
            id: idAsiento,
            numero: `${fila}${letras[i]}`,
            tipo: 'normal', 
            precio: Number(this.PRECIO_ECO), // ASEGURAR NÚMERO
            ocupado: sillasOcupadasIds.includes(idAsiento)
          })
        }
      }
